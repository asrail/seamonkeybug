...
const FilePicker =  CCIN("@mozilla.org/filepicker;1", "nsIFilePicker");
...
Firebug.CSSStyleSheetPanel.prototype = extend(Firebug.SourceBoxPanel,
{
    ...
    toggleEditing: function()
    {
        ...
        else
        {
            ...
            this.stylesheetEditor.styleSheet = styleSheet;
            var strules = this.getStyleSheetRules(this.context, styleSheet);
            var strulestr = [];
            for(var i = 0; i < strules.length; i++) strulestr.push(strules[i].rule.cssText);
            css = strulestr.map(this.translateName).join("\n\n");
     ...
     saveToFile: function()
     {
         ...
         if (this.editing)
             css = Firebug.Editor.getCSSValue();
         else
         {
 			if (!this.location)
 				return;
             ...
             css = getStyleSheetCSS(styleSheet, this.context);
             var strules = this.getStyleSheetRules(this.context, styleSheet);
             var strulestr = []; for(var i = 0; i < strules.length; i++) strulestr.push(strules[i].rule.cssText);
             css = strulestr.map(this.translateName).join("\n\n");
         }

         FilePicker.init(window, "Save CSS to file...", nsIFilePicker.modeSave);
         FilePicker.appendFilter("CSS files", "*.css");
         var res = FilePicker.show();
         if (res != nsIFilePicker.returnCancel) {
             var stream = CCIN("@mozilla.org/network/safe-file-output-stream;1",
                               "nsIFileOutputStream");
             stream.init(FilePicker.file, -1, -1, 0); // write, create, truncate
             stream.write(css, css.length);
